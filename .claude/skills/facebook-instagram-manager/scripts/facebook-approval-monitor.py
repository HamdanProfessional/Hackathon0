...
Watches the /Approved/ folder for approved Facebook posts and publishes them.

This is the human-in-the-loop component - it only posts after you approve.

Usage:
    python facebook-approval-monitor.py --vault .
"""
...
class FacebookApprovalMonitor:
    """
    Monitors the Approved/ folder for Facebook posts using polling.
    """
    ...
    def __init__(self, vault_path: str, dry_run: bool = False):
        ...
        # Check FACEBOOK_DRY_RUN environment variable, default to dry_run parameter
        env_dry_run = os.getenv('FACEBOOK_DRY_RUN', 'true').lower() == 'true'
    ...
    def check_for_updates(self) -> list:
        """Check for newly approved Facebook posts."""
        updates = []
        ...
        files = list(self.approved_folder.glob("FACEBOOK_POST_*.md"))
        ...
        for filepath in files:
            ...
            print(f"\n[OK] Detected approved Facebook post: {filepath.name}")
    ...
    def process_approved_post(self, filepath: Path):
        """
        Process an approved Facebook post.
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print(f"FACEBOOK POST DETAILS:")
            print(f"{'='*60}")
            print(f"Content:\n{post_details.get('content', '')[:200]}...")
            print(f"{'='*60}\n")

            # Log the action
            self._log_action("facebook_post_approved", {
                "file": filepath.name,
            ...
            # Publish to Facebook
            print("[INFO] Publishing to Facebook...")
            success = self._publish_to_facebook(post_details)

            if success:
                print("[OK] Successfully published to Facebook!")
                self._log_action("facebook_post_published", {
                    "file": filepath.name,
                    "timestamp": datetime.now().isoformat(),
                    "result": "success"
                })
                ...
                self._log_action("facebook_post_failed", {
                    "file": filepath.name,
                    "timestamp": datetime.now().isoformat(),
                    "result": "failed"
                })
            ...
            self._log_action("facebook_post_error", {
                "file": filepath.name,
                "error": str(e),
                "timestamp": datetime.now().isoformat()
            })
    ...
    def _publish_to_facebook(self, post_details: dict) -> bool:
        """
        Publish content to Facebook using the Facebook MCP server wrapper.
        ...
        try:
            ...
            mcp_wrapper = vault_root / "mcp-servers" / "facebook-mcp" / "call_post_tool.js"

            if not mcp_wrapper.exists():
                print(f"[ERROR] Facebook MCP wrapper not found: {mcp_wrapper}")
                return False
            ...
            if result.stdout:
                try:
                    ...
                    print(f"[MCP] {response.get('message', 'Posted to Facebook')}")
            ...
            print("[ERROR] Facebook MCP call timed out")
            return False
        except Exception as e:
            print(f"[ERROR] Error calling Facebook MCP: {e}")
            return False

    def _generate_summary(self, post_details: dict):
        """
        Generate a summary of the published Facebook post.
        ...
        try:
            ...
            summary_file = briefings_folder / f"Facebook_Post_Summary_{timestamp}.md"
...
"""
    ...
    def _log_action(self, action: str, details: dict):
        ...
        log_entry = {
            "timestamp": datetime.now().isoformat(),
            "component": "facebook_approval_monitor",
            "action": action,
            "details": details
        }
            ...
            while self._is_running:
                ...
                try:
                    ...
                    if updates:
                        ...
                        print("[INFO] Waiting for approved Facebook posts...")

                except Exception as e:
                    print(f"[ERROR] Error in main loop: {e}")

        except KeyboardInterrupt:
            print("\n\n[INFO] Stopping Facebook approval monitor...")
            self._is_running = False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monitor /Approved/ folder and publish Facebook posts"
    )
    ...
    # Create monitor
    monitor = FacebookApprovalMonitor(args.vault, args.dry_run)

    print("=" * 60)
    print("Facebook Approval Monitor")
    print("=" * 60)
    ...
    print("=" * 60)
    print("\n[INFO] Waiting for approved Facebook posts...")
