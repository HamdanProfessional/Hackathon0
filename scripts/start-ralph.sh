...
if [ ! -d "$RALPH_DIR" ]; then
  echo "❌ Error: Ralph skill directory not found at $RALPH_DIR"
  exit 1
fi

# Check if prd.json exists
if [ ! -f "$RALPH_DIR/prd.json" ]; then
  echo "❌ No task list found ($RALPH_DIR/prd.json)"
  echo ""
  echo "Please create a task list first:"
  echo "   1. Edit $RALPH_DIR/prd.json with your tasks"
  echo "   2. Or use $RALPH_DIR/prd.json.example as template"
  echo "   3. See $RALPH_DIR/README.md for help"
  exit 1
fi
...
echo "   Prompt: $RALPH_DIR/prompt-ai-employee.md"
echo ""
echo "⚠️  Ralph will:"
...
if [ $RALPH_EXIT -eq 0 ]; then
  ...
  echo "   Check $RALPH_DIR/progress.txt for details"
fi
