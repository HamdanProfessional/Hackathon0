...
class InstagramApprovalMonitor:
    ...
    def _publish_to_instagram(self, post_details: dict) -> bool:
        """
        Publish content to Instagram using instagram_poster_v2.py
        ...
        try:
            ...
            # Resolve absolute path first to handle relative paths correctly
            ...
            poster_script = vault_root / "scripts" / "social-media" / "instagram_poster_v2.py"

            print(f"[DEBUG] vault_path_abs: {vault_path_abs}")
            print(f"[DEBUG] vault_root: {vault_root}")
            print(f"[DEBUG] poster_script: {poster_script}")
            print(f"[DEBUG] poster_script.exists(): {poster_script.exists()}")

            if not poster_script.exists():
                print(f"[ERROR] Instagram poster script not found: {poster_script}")
                return False
            ...
            # Call the instagram poster script
            # Set PYTHONIOENCODING to utf-8 to handle Unicode characters
            import os
            env = os.environ.copy()
            ...
            print("[ERROR] Instagram poster timed out")
            return False
        except Exception as e:
            print(f"[ERROR] Error running instagram_poster: {e}")
            return False
