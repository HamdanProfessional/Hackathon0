...
if [ ! -f "$PRD_FILE" ]; then
  echo "❌ No task list found ($PRD_FILE)"
  echo ""
  echo "To get started:"
  echo "   1. Create prd.json in $RALPH_DIR"
...
echo "   cat $PRD_FILE | jq '.'"
echo ""
echo "View specific task:"
echo "   cat $PRD_FILE | jq '.userStories[0]'"
echo ""
echo "Mark task complete:"
echo "   # Edit $PRD_FILE, set passes: true for the task"
echo ""
echo "Reset all tasks:"
echo "   # Edit $PRD_FILE, set all passes: false"
echo ""
